...
const TargetGameRange = ({
  ...
  const [dataRange, setDataRange] = useState<RangeType>(() => {
    return dataTarget[typeAccount][`${keyCurrent}_Detail`].range || {}
  })
    ...
    setDataTarget((prev) => {
      return {
        ...
        [typeAccount]: {
          ...prev[typeAccount],
          [`${keyCurrent}_Detail`]: {
            ...dataTarget[typeAccount][`${keyCurrent}_Detail`],
  ...
  return (
    <div className="absolute w-[300px] h-[350px] right-[12px] bottom-4 bg-layout-color z-[60] border border-gray-400 flex flex-col shadow-2xl">
      <div>
        <p
          className="text-start font-bold underline text-red-color leading-none p-[1px] cursor-pointer"
          onClick={() => setShowGameRange((prev: boolean) => !prev)}
        >
          Close
        </p>

        <p className=" pb-2 text-center font-bold text-lg text-blue-900">{`${label} - Game Range`}</p>
      ...
      <div className="flex-1 px-4">
        <BoxLabel label="Game Type" className="w-full p-2 flex flex-col">
          ...
          <div className="bg-white border border-gray-600  grow h-[100px] p-[1px] ">
      ...
      <div className="flex gap-3 justify-center items-center pt-2 pb-3">
...
export default React.memo(TargetGameRange)
